...
const App: React.FC = () => {
	...
	useEffect(() => {
		const loadCache = async () => {
			const cachedImages = await loadImagesFromCache();
			if (cachedImages.length > 0) {
				setState((prev) => ({ ...prev, generatedImages: cachedImages }));
			}
			setCacheLoaded(true);
		};
		loadCache();
	...
	// Save generated images to IndexedDB when they change
	useEffect(() => {
		if (cacheLoaded) {
			saveImagesToCache(state.generatedImages);
		}
	}, [state.generatedImages, cacheLoaded]);
		...
		<div className="space-y-5">
			<section className="space-y-1">
				<header className="flex items-center justify-between px-1">
					<label className="text-[10px] font-black text-monstera-800 uppercase tracking-widest">
						Prompt
					</label>
			...
			<section className="space-y-1.5">
				<label className="text-[10px] font-black text-monstera-800 uppercase tracking-widest px-1 block">
					References
				</label>
				<div className="grid grid-cols-2 gap-1.5">
					{state.sourceImages.map((img) => (
						<div
							key={img.id}
							className="relative group aspect-square rounded-md overflow-hidden border border-monstera-200 bg-monstera-50 shadow-sm transition-all hover:border-monstera-300"
						>
			...
			<section className="bg-white border border-monstera-200 rounded-md shadow-md overflow-hidden">
				<div className="bg-monstera-50 border-b border-monstera-200 px-3 py-2 flex items-center gap-1.5">
					<svg
						className="w-3.5 h-3.5 text-monstera-800"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
					...
					<span className="text-[10px] font-black text-monstera-800 uppercase tracking-widest">
						Generation Settings
					</span>
				</div>
				...
				<div className="p-3.5 space-y-3">
						...
						<div className="space-y-1">
							...
							<div className="relative">
								<select
									...
									onChange={(e) =>
										setState((p) => ({ ...p, aspectRatio: e.target.value }))
									}
						...
						<div className="space-y-1">
							...
							<div className="relative">
								<select
									...
									onChange={(e) =>
										setState((p) => ({ ...p, resolution: e.target.value }))
									}
									className="w-full bg-white border border-monstera-200 text-[11px] font-bold rounded-md px-2.5 py-1.5 outline-none cursor-pointer hover:border-monstera-300 appearance-none shadow-sm"
								>
									{RESOLUTIONS.map((r) => (
					...
					<div className="space-y-2">
						<div className="flex items-center gap-2 px-1">
							<label className="text-[9px] text-monstera-800 font-black uppercase tracking-widest">
								Style Seed
							</label>
							<button
						...
						<div className="relative">
							<input
								...
								onChange={(e) =>
									setState((p) => ({
										...p,
										styleCode: e.target.value.replace(/\D/g, ""),
									}))
								}
								className={`w-full bg-white border border-monstera-200 text-xs font-mono font-bold rounded-md pl-3 pr-8 py-1.5 outline-none focus:border-monstera-400 shadow-sm transition-all ${
									state.randomizeEachTime
										? "bg-monstera-50 text-monstera-400 select-none"
										: ""
								}`}
								placeholder={
									state.randomizeEachTime
										? "Randomizing each run..."
										: "Optional Seed"
								}
							/>
							{!state.randomizeEachTime && (
						...
						<label className="flex items-center gap-2.5 cursor-pointer group px-1 select-none">
							<div className="relative">
								<input
									...
									onChange={(e) =>
										setState((p) => ({
											...p,
											randomizeEachTime: e.target.checked,
										}))
									}
									className="sr-only peer"
								/>
								<div className="w-7 h-3.5 bg-monstera-300 rounded-full peer-checked:bg-ink transition-colors"></div>
							...
							<span
								className={`text-[9px] font-bold uppercase tracking-wide transition-colors ${
									state.randomizeEachTime
										? "text-ink"
										: "text-monstera-600 group-hover:text-ink"
								}`}
							>
								New seed every run
							</span>
						...
						<div className="border-t border-monstera-200 mt-2 pt-2">
							<label className="flex items-center gap-2.5 cursor-pointer group px-1">
								...
								<div className="flex flex-col">
									<span className="text-[9px] font-black uppercase tracking-wide text-monstera-700 group-hover:text-ink transition-colors">
										Search Grounding
									</span>
									<span className="text-[7px] font-bold text-monstera-500 uppercase tracking-widest leading-none">
										Slower generation
									</span>
								</div>
							</label>
	...
	return (
		<div className="flex h-screen overflow-hidden bg-white text-ink font-sans selection:bg-monstera-200">
			...
			>
				...
				<div className="p-4 border-t border-monstera-200 bg-paper/80 backdrop-blur-xl">
					<button
						...
						className="w-full py-3 px-6 bg-ink hover:bg-monstera-800 text-white font-[900] text-[13px] uppercase tracking-[0.2em] border-2 border-ink rounded-md transition-all shadow-[5px_5px_0_rgba(0,0,0,0.2)] active:shadow-none active:translate-x-0.5 active:translate-y-0.5 disabled:opacity-20 disabled:cursor-not-allowed disabled:grayscale"
					>
						Generate
					</button>
				</div>
			...
			<div
				className="w-1 bg-transparent hover:bg-monstera-400/60 transition-colors z-30 hidden lg:block cursor-col-resize active:bg-monstera-500 h-full"
			...
			<main className="flex-1 h-full overflow-y-auto custom-scrollbar bg-white relative flex flex-col">
				...
				<div className="lg:hidden sticky top-0 z-40 bg-white/95 backdrop-blur border-y border-monstera-200 shadow-sm p-3 flex gap-3 items-center transition-all">
					...
					<button
						onClick={handleGenerate}
						disabled={!state.prompt.trim()}
						className="bg-ink text-white font-black text-[10px] uppercase tracking-widest px-4 py-2.5 rounded-md border border-ink shadow-[2px_2px_0_rgba(0,0,0,0.2)] active:shadow-none active:translate-x-[1px] active:translate-y-[1px] disabled:opacity-50 disabled:grayscale"
					>
						Go
					</button>
				</div>
				...
				{isMobileMenuOpen && (
					<div className="lg:hidden fixed inset-0 z-50 bg-paper flex flex-col animate-fadeIn">
						<div className="flex items-center justify-between p-4 border-b border-monstera-200 bg-white">
							<span className="font-black uppercase tracking-widest text-xs text-ink">
								Configuration
							</span>
							<button
						...
						<div className="p-5 border-t border-monstera-200 bg-paper absolute bottom-0 left-0 right-0">
							<button
								onClick={handleGenerate}
								disabled={!state.prompt.trim()}
								className="w-full py-3.5 px-6 bg-ink text-white font-[900] text-[13px] uppercase tracking-[0.2em] border-2 border-ink rounded-md transition-all shadow-[4px_4px_0_rgba(0,0,0,0.2)] active:shadow-none active:translate-x-0.5 active:translate-y-0.5 disabled:opacity-20 disabled:grayscale"
							>
								Generate
							</button>
						</div>
					...
					<header className="hidden lg:flex flex-col md:flex-row md:items-end justify-between gap-4 px-1">
						<div className="space-y-1">
							<div className="flex items-center gap-2">
								...
								<h2 className="text-[11px] font-[900] uppercase tracking-[0.3em] text-ink">
									Gallery
								</h2>
							</div>
						</div>
						...
						<div className="flex flex-wrap items-center gap-4 hidden lg:flex">
							<div className="flex items-center gap-1.5 bg-monstera-50 p-1 rounded-md border border-monstera-200 shadow-sm">
								<span className="text-[8px] font-black uppercase opacity-40 px-2">
									Grid
								</span>
								<div className="flex gap-1">
									{[1, 2, 3, 4, 5].map((n) => (
										<button
											...
											className={`w-7 h-7 rounded-md font-black text-[10px] transition-all flex items-center justify-center ${
												gridCols === n
													? "bg-white text-ink shadow-sm border border-monstera-200"
													: "text-monstera-600 hover:text-ink hover:bg-white/50"
											}`}
										>
											{n}
										</button>
							...
							{state.generatedImages.length > 0 && (
								<>
										...
										onClick={async () => {
											...
											await Promise.all(
												successImages.map(async (img) => {
													...
													const stylePart = img.styleCode
														? `-${img.styleCode}`
														: "";
													const baseFilename = `${img.id}${stylePart}${
														slug ? "-" + slug : ""
													}`;
									...
									<button
										onClick={clearCache}
										className="flex items-center gap-2 px-4 py-2 bg-white text-monstera-600 hover:text-red-600 font-black text-[9px] uppercase tracking-widest rounded-md border border-monstera-200 hover:border-red-300 shadow-sm transition-all active:scale-95"
										title="Clear all cached images"
									>
										<svg
											className="w-3 h-3"
											fill="none"
								...
								>
									<div
										className={`relative bg-monstera-50 cursor-zoom-in ${
											gridCols === 1 ? "" : "aspect-square"
										}`}
										style={
											gridCols === 1 && image.status !== "success"
												? getLoadingAspectRatio(image.aspectRatio)
												: undefined
										}
									...
									>
										...
										{image.status === "error" && (
											<div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
												...
												<h4 className="text-red-700 font-[900] uppercase text-[9px] mb-2 tracking-[0.2em]">
													Error
												</h4>
												<p className="text-[8px] font-bold text-red-500 leading-relaxed max-w-[150px]">
													{image.error}
												</p>
											</div>
										)}
									...
									<div className="px-3 py-2.5 flex flex-col gap-2 border-t border-monstera-200 bg-white">
										<div className="flex items-center gap-3">
											<p
												className="text-[11px] font-bold text-ink leading-snug line-clamp-1 flex-1"
												title={image.prompt}
											>
												{image.prompt}
											</p>
											<div className="flex gap-1 shrink-0">
												...
												{image.url && (
													<a
														href={image.url}
														download={`${image.id}${
															image.styleCode ? "-" + image.styleCode : ""
														}${
															slugify(image.prompt)
																? "-" + slugify(image.prompt)
																: ""
														}.jpg`}
														className="p-2 text-monstera-400 hover:text-ink hover:bg-monstera-100 rounded-md transition-all border border-transparent hover:border-monstera-200"
										...
										{image.groundingMetadata?.groundingChunks && (
											<div className="flex flex-wrap gap-1.5 mt-1">
												{image.groundingMetadata.groundingChunks.map(
													(chunk: any, i: number) =>
														chunk.web?.uri && (
															<a
																key={i}
			...
			{showSeedHelp && (
				<StyleSeedHelpModal onClose={() => setShowSeedHelp(false)} />
			)}
		</div>
	);
};
