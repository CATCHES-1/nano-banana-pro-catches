	...
	return (
		<div className="fixed inset-0 z-[150] flex flex-col md:flex-row animate-fadeIn bg-ink">
			...
			>
				...
				{/* Image Content */}
				<div
					className="relative w-full h-full p-4 md:py-12 md:px-24 flex items-center justify-center"
					...
					<div className="relative w-full h-full flex items-center justify-center">
						{isCompareMode && originalImage ? (
							...
							>
								<img
									src={generatedImage}
									className="max-w-full max-h-full object-contain pointer-events-none select-none block mx-auto"
									draggable={false}
								/>
								<div
								...
								>
									<div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-8 h-8 bg-white border-2 border-ink rounded-full shadow-lg flex items-center justify-center">
										<svg
			...
			<div
				className={`fixed bottom-0 left-0 right-0 z-[160] bg-ink border-t border-white/10 shadow-2xl transition-all duration-300 ease-in-out md:static md:w-[400px] md:h-full md:border-t-0 md:border-l flex flex-col ${
					isDrawerOpen ? "h-[80vh]" : "h-[60px]"
				} md:h-full`}
				onClick={(e) => {
					e.stopPropagation();
			...
			>
				...
				>
					<div className="flex items-center gap-2 text-monstera-400">
				...
				<div className="flex-1 overflow-y-auto p-6 custom-scrollbar min-h-0 space-y-6">
					<div className="space-y-2">
						...
						<p className="text-monstera-100/90 font-mono text-sm leading-relaxed whitespace-pre-wrap selection:bg-white selection:text-ink">
							{displayedPrompt}
						</p>
					</div>
					...
					<div className="space-y-4 pt-4 border-t border-white/10">
						<div className="grid grid-cols-2 gap-4">
							<div className="space-y-1">
								...
								<p className="text-monstera-100 font-mono text-xs">
									{resolution || "Default"}
								</p>
							</div>
							<div className="space-y-1">
								...
								<div className="flex items-center justify-between">
									...
									<label className="flex items-center gap-1.5 cursor-pointer group">
										<div className="relative flex items-center">
											...
											/>
											<div className="w-6 h-3 bg-white/20 rounded-full peer-checked:bg-white transition-colors"></div>
											<div className="absolute left-0.5 w-2 h-2 bg-white rounded-full transition-transform peer-checked:translate-x-3 peer-checked:bg-ink"></div>
										</div>
										<span className="text-[8px] text-monstera-400 font-bold uppercase group-hover:text-monstera-300 transition-colors">
											Show full prompt
										</span>
									</label>
								</div>
						...
						{groundingMetadata && (
							<div className="space-y-1">
								...
								<span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-white text-ink">
									Enabled
								</span>
							</div>
						)}
				...
				<div className="p-6 border-t border-white/5 flex flex-col gap-3 shrink-0 bg-ink">
					{originalImage && (
						<button
							...
							className={`w-full py-3 rounded-lg text-xs font-black uppercase tracking-widest border transition-all ${
								isCompareMode
					...
					<button
						...
						className="w-full py-3 rounded-lg text-xs font-black uppercase tracking-widest border-2 bg-white text-ink border-white hover:bg-monstera-200 hover:border-monstera-200 transition-all shadow-lg active:scale-95"
					>
						Download Image
					</button>
				</div>
